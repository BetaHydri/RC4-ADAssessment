...
A service ticket can be AES-encrypted while the session key inside it uses RC4 — or vice versa. RC4 session keys represent a separate RC4 dependency that **does not trigger KDCSVC events 201–209**, because the KDC's disablement logic targets ticket encryption decisions, not session key negotiation. These RC4 session keys are only visible through Security event log analysis (4768/4769), which is why RC4-ADAssessment includes event log correlation as a critical detection layer.

> **Reference:** [MS-KILE — Kerberos Protocol Extensions](https://learn.microsoft.com/openspecs/windows_protocols/ms-kile/2a32282e-6ab7-4f56-b532-870c74e1c653)
